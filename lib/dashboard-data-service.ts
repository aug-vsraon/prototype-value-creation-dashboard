...
export function fetchDashboardData(from: string, to: string): DashboardData {
  ...
  const getOrCreate = (weekIso: string, label: string) => {
    if (!weeklyMap.has(weekIso)) {
      weeklyMap.set(weekIso, {
        ...
        weekIso, dc: 0, tt: 0, cs: 0, lb: 0, as: 0, qt: 0, total: 0, interactions: 0,
      })
    }
  ...
  const weeklyStacked = Array.from(weeklyMap.values())
    ...
    .map(w => ({ ...w, dc: Math.round(w.dc), tt: Math.round(w.tt), cs: Math.round(w.cs), lb: Math.round(w.lb), as: Math.round(w.as), qt: Math.round(w.qt), total: Math.round(w.dc + w.tt + w.cs + w.lb + w.as + w.qt) }))
